...
class GodotServer {
  ...
  private async cleanup() {
    ...
    for (const [instanceId, proc] of this.activeProcesses) {
      this.logDebug(`Killing active Godot process (instance: ${instanceId})`);
      proc.process.kill();
    }
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              instanceId: {
                type: 'string',
                description: "Optional: identifier for this instance (e.g. 'host', 'client1'). Defaults to 'default'. Each distinct instanceId runs as its own independent Godot process, so multiple instances can run and be debugged at once.",
              },
              extraArgs: {
                type: 'array',
                items: { type: 'string' },
                description: 'Optional: extra CLI args passed to this instance (e.g. ["--position=1920,0"] to place a second window, or ["--", "--net-host=8910"] for user args)',
              },
            },
            required: ['projectPath'],
          },
        },
        {
          ...
          description: 'Get the current debug output and errors for a given instance',
          inputSchema: {
            type: 'object',
            properties: {
              instanceId: {
                type: 'string',
                description: "Optional: instance identifier used in run_project. Defaults to 'default'.",
              },
            },
            required: [],
          },
        ...
        {
          name: 'list_instances',
          description: 'List all currently active Godot process instance IDs',
          inputSchema: {
            type: 'object',
            properties: {},
            required: [],
  ...
  private async handleRunProject(args: any) {
    ...
    try {
      ...
      const instanceId: string = args.instanceId || 'default';

      // Kill any existing process for this instance only — other instances
      // (e.g. a netfox host/client pair) keep running untouched.
      const existing = this.activeProcesses.get(instanceId);
      if (existing) {
        this.logDebug(`Killing existing Godot process for instance '${instanceId}' before starting a new one`);
      ...
      this.logDebug(`Running Godot project (instance: ${instanceId}): ${args.projectPath} ${cmdArgs.join(' ')}`);
      ...
      process.stdout?.on('data', (data: Buffer) => {
        ...
        lines.forEach((line: string) => {
          if (line.trim()) this.logDebug(`[Godot stdout][${instanceId}] ${line}`);
        });
      });
      ...
      process.stderr?.on('data', (data: Buffer) => {
        ...
        errors.push(...lines);
        lines.forEach((line: string) => {
          if (line.trim()) this.logDebug(`[Godot stderr][${instanceId}] ${line}`);
        });
      });

      process.on('exit', (code: number | null) => {
        this.logDebug(`Godot process exited with code ${code} (instance: ${instanceId})`);
        const current = this.activeProcesses.get(instanceId);
      ...
      this.activeProcesses.set(instanceId, { process, output, errors });

      return {
        content: [
          {
            type: 'text',
            text: `Godot project started in debug mode (instance: ${instanceId}). Use get_debug_output with instanceId to see output.`,
          },
        ],
      };
    } catch (error: unknown) {
  ...
  private async handleGetDebugOutput(args: any) {
    ...
    if (!proc) {
      return this.createErrorResponse(
        `No active Godot process for instance '${instanceId}'.`,
        [
          'Use run_project to start a Godot project first',
          'Check if the Godot process crashed unexpectedly',
          'Pass the same instanceId used in run_project',
        ]
      );
    }
  ...
  private async handleStopProject(args: any) {
    ...
    if (!proc) {
      return this.createErrorResponse(
        `No active Godot process for instance '${instanceId}' to stop.`,
        [
          'Use run_project to start a Godot project first',
    ...
    this.logDebug(`Stopping active Godot process (instance: ${instanceId})`);
    proc.process.kill();
    this.activeProcesses.delete(instanceId);
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              message: `Godot project stopped (instance: ${instanceId})`,
              finalOutput: proc.output,
              finalErrors: proc.errors,
            },
            null,
            2
          ),
  ...
  /**
   * Handle the list_instances tool
   */
  private async handleListInstances() {
    return {
      content: [
        {
          type: 'text',
          text: JSON.stringify(Array.from(this.activeProcesses.keys()), null, 2),
        },
      ],
    };
  }
