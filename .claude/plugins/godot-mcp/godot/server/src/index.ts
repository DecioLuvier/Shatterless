...
class GodotServer {
  ...
  private runtimeScriptPath: string;
  ...
  constructor(config?: GodotServerConfig) {
    ...
    if (debugMode) console.error(`[DEBUG] Runtime script path: ${this.runtimeScriptPath}`);
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          description:
            "Boot the project's main scene in a real window, let it render for a few frames, and return a PNG screenshot of the running game. Nothing is added to the project.",
          inputSchema: {
            type: 'object',
            properties: {
              projectPath: {
   ...
   */
  private async runtimePreflight(args: any): Promise<{ ok: true; args: any } | { ok: false; response: any }> {
    args = this.normalizeParameters(args);
   ...
   */
  private async handleCaptureScreenshot(rawArgs: any) {
    ...
    const params: OperationParams = { outputPath };
    if (args.scenePath) params.scenePath = args.scenePath;
    if (typeof args.waitFrames === 'number') params.waitFrames = args.waitFrames;

    try {
      const { stdout, stderr } = await this.executeRuntimeOperation(
        'screenshot',
        params,
        args.projectPath
      );
   ...
   */
  private async handleSendGameCommand(rawArgs: any) {
    const pre = await this.runtimePreflight(rawArgs);
    if (!pre.ok) return pre.response;
    const args = pre.args;
    ...
    const params: OperationParams = { command: args.command };
    if (args.scenePath) params.scenePath = args.scenePath;
    if (typeof args.waitFrames === 'number') params.waitFrames = args.waitFrames;
    ...
    try {
      ...
      return this.createErrorResponse(
        `Failed to send game command: ${error?.message || 'Unknown error'}`,
        [
          'Ensure Godot is installed correctly',
          'Check if the GODOT_PATH environment variable is set correctly',
          'Verify the project path is accessible',
