...
class GodotServer {
    ...
    constructor(config) {
        ...
        this.runtimeScriptPath = join(__dirname, 'scripts', 'godot_runtime.gd');
        if (debugMode)
            console.error(`[DEBUG] Operations script path: ${this.operationsScriptPath}`);
        if (debugMode)
            console.error(`[DEBUG] Runtime script path: ${this.runtimeScriptPath}`);
    ...
    /**
     * Boot the project's main scene in a real window using the bundled runtime
     * harness (godot_runtime.gd) and run a single runtime operation against it.
     *
     * Nothing is written into the Godot project: the harness is passed via
     * `--script` from the plugin directory, runs for a few frames, performs the
     * operation ("screenshot" or "command"), then quits.
     *
     * @param operation "screenshot" | "command"
     * @param params Operation params (snake_case keys, JSON-serializable)
     * @param projectPath Path to the Godot project
     * @param timeoutMs Max time to wait for Godot to finish
     */
    async executeRuntimeOperation(operation, params, projectPath, timeoutMs = 60000) {
        this.logDebug(`Executing runtime operation: ${operation} in project: ${projectPath}`);
        const snakeCaseParams = this.convertCamelToSnakeCase(params);
        if (!this.godotPath) {
            await this.detectGodotPath();
            if (!this.godotPath) {
        ...
        this.logDebug(`Executing (runtime): ${this.godotPath} ${args.join(' ')}`);
        try {
            const { stdout, stderr } = await execFileAsync(this.godotPath, args, {
                timeout: timeoutMs,
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                },
                {
                    ...
                    description: "Boot the project's main scene in a real window, let it render for a few frames, and return a PNG screenshot of the running game. Nothing is added to the project.",
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectPath: {
                            ...
                            scenePath: {
                                type: 'string',
                                description: 'Optional: res:// path of a scene to boot instead of the configured main scene',
                            },
                            outputPath: {
                                type: 'string',
                                description: 'Optional: absolute path to write the PNG to (defaults to a temp file)',
                            },
                            waitFrames: {
                                type: 'number',
                                description: 'Optional: frames to render before capturing (default 45)',
                            },
                        },
                        required: ['projectPath'],
                    },
                },
                {
                    ...
                    description: "Boot the project's main scene in a real window and evaluate a GDScript expression against it. The expression has access to `tree` (SceneTree), `root` (Window), and `scene` (the running main scene root). Use it to call methods, read state, or drive the game, e.g. `scene.get_node(\"Player\").global_position` or `scene.some_method(1, 2)`. Returns the printed output and the expression result. Stateless: a fresh instance boots per call.",
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectPath: {
                            ...
                            command: {
                                type: 'string',
                                description: 'GDScript expression to evaluate (names available: tree, root, scene)',
                            },
                            scenePath: {
                                type: 'string',
                                description: 'Optional: res:// path of a scene to boot instead of the configured main scene',
                            ...
                            waitFrames: {
                                type: 'number',
                                description: 'Optional: frames to render before running the command (default 45)',
                            },
                        },
    ...
    async handleCreateScene(args) {
        ...
        if (!this.validateClassName(rootNodeType)) {
            return this.createErrorResponse('Invalid rootNodeType', ['rootNodeType must be a built-in Godot class name (no paths, no file extensions)']);
        }
        try {
            // Check if the project directory exists and contains a project.godot file
            const projectFile = join(args.projectPath, 'project.godot');
    ...
    async handleAddNode(args) {
        ...
        if (!this.validateClassName(args.nodeType)) {
            return this.createErrorResponse('Invalid nodeType', ['nodeType must be a built-in Godot class name (no paths, no file extensions)']);
        }
        try {
            // Check if the project directory exists and contains a project.godot file
            const projectFile = join(args.projectPath, 'project.godot');
    ...
    /**
     * Shared preflight for the runtime tools: normalize args, validate the
     * project path, ensure Godot is resolvable.
     */
    async runtimePreflight(args) {
        args = this.normalizeParameters(args);
        if (!args.projectPath) {
            return {
                ok: false,
                response: this.createErrorResponse('Project path is required', [
        ...
        if (!this.validatePath(args.projectPath)) {
            return {
                ok: false,
                response: this.createErrorResponse('Invalid project path', [
        ...
        if (args.scenePath && !this.validatePath(args.scenePath)) {
            return {
                ok: false,
                response: this.createErrorResponse('Invalid scene path', [
                    'Provide a valid res:// path without ".."',
                ]),
            };
        }
        const projectFile = join(args.projectPath, 'project.godot');
        if (!existsSync(projectFile)) {
            return {
                ok: false,
                response: this.createErrorResponse(`Not a valid Godot project: ${args.projectPath}`, [
                    'Ensure the path points to a directory containing a project.godot file',
                ]),
            };
        }
        if (!this.godotPath) {
            ...
            if (!this.godotPath) {
                return {
                    ok: false,
                    response: this.createErrorResponse('Could not find a valid Godot executable path', [
                        'Ensure Godot is installed correctly',
                        'Set GODOT_PATH environment variable to specify the correct path',
                    ]),
                };
     ...
     */
    async handleCaptureScreenshot(rawArgs) {
        const pre = await this.runtimePreflight(rawArgs);
        if (!pre.ok)
            return pre.response;
        const args = pre.args;
        const outputPath = args.outputPath ||
            join(tmpdir(), `godot-mcp-screenshot-${Date.now()}.png`);
        if (!args.outputPath && outputPath.includes('..')) {
            return this.createErrorResponse('Invalid output path', ['Provide a valid absolute path']);
        }
        const params = { outputPath };
        if (args.scenePath)
            params.scenePath = args.scenePath;
        if (typeof args.waitFrames === 'number')
            params.waitFrames = args.waitFrames;
        try {
            const { stdout, stderr } = await this.executeRuntimeOperation('screenshot', params, args.projectPath);
            if (!existsSync(outputPath)) {
                return this.createErrorResponse(`Screenshot was not produced.\n\nGodot stdout:\n${stdout}\n\nGodot stderr:\n${stderr}`, [
                    'Ensure the project has a main scene set (or pass scenePath)',
                    'Increase waitFrames if the scene needs longer to initialize',
                    'Check that this machine can open a rendering window',
                ]);
            }
     ...
     */
    async handleSendGameCommand(rawArgs) {
        const pre = await this.runtimePreflight(rawArgs);
        if (!pre.ok)
            return pre.response;
        const args = pre.args;
        if (!args.command || typeof args.command !== 'string') {
            return this.createErrorResponse('A "command" string is required', [
                'Provide a GDScript expression, e.g. scene.get_node("Player").global_position',
            ]);
        }
        ...
        if (typeof args.waitFrames === 'number')
            params.waitFrames = args.waitFrames;
        try {
            const { stdout, stderr } = await this.executeRuntimeOperation('command', params, args.projectPath);
            const failed = /\[ERROR\] command:/.test(stderr);
            return {
                content: [
                    {
                        type: 'text',
            ...
            return this.createErrorResponse(`Failed to send game command: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
